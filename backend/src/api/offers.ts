...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      let filtered = [...INITIAL_OFFERS];
      if (medicineId) filtered = filtered.filter((o) => o.medicineId === medicineId);
      if (packId) filtered = filtered.filter((o) => o.packId === packId);
      if (pharmacyId) filtered = filtered.filter((o) => o.pharmacyId === pharmacyId);
      if (inStockOnly === 'true') filtered = filtered.filter((o) => o.inStock);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const matching = INITIAL_OFFERS.filter(
        (o) => o.medicineId === medicineId && o.packId === packId && o.inStock
      ).sort((a, b) => a.price - b.price);
      res.json({ offer: matching[0] || null });
    }
  })
);
