...
export const Header: React.FC<HeaderProps> = ({ theme, onToggleTheme, onOpenCart }) => {
  ...
  const getNavLinks = () => {
    switch (currentRole) {
      case 'pharmacy':
        return [
          { id: 'pharmacy_orders', label: t('nav.pharmacy_orders', 'Fulfillment Queue'), icon: Building2 },
          { id: 'pharmacy_rx', label: t('nav.pharmacy_rx', 'Prescription Review'), icon: FileText },
          { id: 'pharmacy_inventory', label: t('nav.pharmacy_inventory', 'Inventory & Prices'), icon: Layers },
          { id: 'pharmacy_profile', label: t('nav.pharmacy_profile', 'Store Profile'), icon: ShieldCheck },
          { id: 'architecture', label: t('nav.architecture', 'Architecture & PRD'), icon: Activity },
        ];
      case 'admin':
        return [
          { id: 'admin_dashboard', label: t('nav.admin_dashboard', 'Operations Dashboard'), icon: ShieldCheck },
          { id: 'admin_audit', label: t('nav.admin_audit', 'Immutable Audit Log'), icon: FileText },
          { id: 'admin_catalog', label: t('nav.admin_catalog', 'Catalog Normalization'), icon: Layers },
          { id: 'architecture', label: t('nav.architecture', 'Architecture & PRD'), icon: Activity },
        ];
      case 'doctor':
        return [
          { id: 'doctor_prescribe', label: t('nav.doctor_prescribe', 'Prescriber Portal'), icon: Stethoscope },
          { id: 'compare', label: t('nav.compare', 'Drug Equivalence Guide'), icon: Pill },
          { id: 'architecture', label: t('nav.architecture', 'Architecture & PRD'), icon: Activity },
        ];
      case 'patient':
      default:
        return [
          { id: 'compare', label: t('nav.compare', 'Compare Medicines'), icon: Pill },
          { id: 'prescriptions', label: t('nav.prescriptions', 'Prescription Vault'), icon: FileText },
          { id: 'orders', label: t('nav.orders', 'Track Orders'), icon: ShieldCheck },
          { id: 'medical_history', label: t('nav.medical_history', 'Medical History'), icon: Activity },
          { id: 'pharmacy_profile', label: t('nav.pharmacy_profile', 'Partner Pharmacies'), icon: Building2 },
          { id: 'architecture', label: t('nav.architecture', 'Architecture & PRD'), icon: Layers },
        ];
    }
  };
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-2 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-xs font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition shadow-xs"
                aria-label="Select Language"
              >
                <Globe className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                <span className="text-[11px] font-bold uppercase">{i18n.language?.slice(0, 2) || 'EN'}</span>
                <ChevronDown className="h-3 w-3 text-zinc-400" />
              </button>

              {langDropdownOpen && (
                <div
                  className="absolute right-0 mt-1 w-36 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xl py-1 z-50 text-zinc-900 dark:text-zinc-100"
                ...
                >
                  ...
                  {languages.map((lang) => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2 text-left text-xs transition hover:bg-zinc-50 dark:hover:bg-zinc-800 ${
                        i18n.language?.startsWith(lang.code) ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 font-bold' : ''
                      }`}
                    >
