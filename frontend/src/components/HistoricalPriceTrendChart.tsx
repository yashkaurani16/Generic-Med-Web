...
export const HistoricalPriceTrendChart: React.FC<HistoricalPriceTrendChartProps> = ({
  ...
  const activeMed = useMemo(() => {
    return medicines.find((m) => m.id === activeMedId) || medicines[0] || null;
  ...
  const currentPack = useMemo(() => {
    if (!activeMed || !activeMed.packs) return null;
    return (
      activeMed.packs.find((p) => p.packId === selectedPackId) || activeMed.packs[0] || null
    );
  ...
  const lowestLiveOffer = useMemo(() => {
    if (!activeMed || !currentPack) return undefined;
    return getLowestComparableOffer(activeMed.id, currentPack.packId);
  }, [getLowestComparableOffer, activeMed, currentPack]);
  ...
  if (!activeMed || !currentPack || !assessment) {
    return (
      <div
        id="historical-price-trends-section"
        className={`rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-8 shadow-sm transition-all ${className}`}
      >
        <div className="flex flex-col items-center justify-center py-10 text-center">
          <div className="h-10 w-10 rounded-full bg-emerald-50 dark:bg-emerald-950/70 border border-emerald-200 dark:border-emerald-800 flex items-center justify-center text-emerald-600 dark:text-emerald-400 mb-3 animate-pulse">
            <Sparkles className="h-5 w-5" />
          </div>
          <h3 className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">
            {medicines.length === 0 ? 'Loading Verified Price Corridors...' : 'Select a medicine to view price trends'}
          </h3>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Analyzing 30-day to 1-year historical pricing and pharmacy partner corridors.
          </p>
        </div>
      </div>
    );
